...
import type { Patient, InfusionSession, OnboardingStatus, PaymentStatus } from "../data/dummy-data";
...
export const PatientDetailsModal: React.FC<PatientDetailsModalProps> = ({ patientId, onClose }) => {
  ...
  const handleOverviewSave = (e: React.FormEvent) => {
    e.preventDefault();
    // Patient object in state is automatically updated on field changes, but we can log notification
  };
  ...
  return (
    <>
      <Dialog open={Boolean(patientId)} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto p-0 gap-0">
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-slate-300">
                  <span className="flex items-center gap-1">
                    <Phone className="h-3.5 w-3.5 text-blue-400" /> {patient.phone}
                  </span>
                  <span className="flex items-center gap-1">
                    <Stethoscope className="h-3.5 w-3.5 text-emerald-400" /> {patient.assignedDoctor}
                  </span>
                  <span className="flex items-center gap-1">
                    <User className="h-3.5 w-3.5 text-purple-400" /> Intern: {patient.allottedIntern || "Unassigned"}
                  </span>
                </div>
              </div>

              <div className="flex flex-col items-start md:items-end gap-1.5 bg-slate-800/80 p-3 rounded-lg border border-slate-700 text-xs">
                <span className="text-slate-400">Financial Ledger Summary</span>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="overview" className="p-6 space-y-6">
              <form onSubmit={handleOverviewSave} className="space-y-4 text-xs">
