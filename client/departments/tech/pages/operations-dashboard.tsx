...
import { milestonesKey, useCreateMilestone, useDeleteMilestone, useListMilestones, useUpdateMilestone } from "@/departments/tech/lib/milestones";
import { useListMemberProfiles } from "@/departments/tech/lib/member-profiles";
import { blockersKey, useCreateBlocker, useDeleteBlocker, useListBlockers, useUpdateBlocker } from "@/departments/tech/lib/blockers";
...
const isSignedOff = (status: string) => status === "signed_off" || status === "sign_off";
...
export default function OperationsDashboard() {
  ...
  const canManageRecords = isCeoOffice || isMember === false;
  ...
  const completedProjects = projects.filter((project) => isSignedOff(project.status)).length;
  const dueSoon = projects.filter((project) => project.deadline && new Date(project.deadline).getTime() < Date.now() + 14 * 86400000 && !isSignedOff(project.status)).length;
  ...
  const refresh = (key: readonly string[]) => queryClient.invalidateQueries({ queryKey: key });
  const openBlockerDialog = (blocker?: typeof openBlockers[number]) => setRecordDialog({ type: "blocker", id: blocker?.id, projectId: blocker ? String(blocker.projectId) : "", title: "", description: blocker?.description ?? "", severity: blocker?.severity ?? "medium", milestoneDate: "" });
  const openMilestoneDialog = (milestone?: typeof upcomingMilestones[number]) => setRecordDialog({ type: "milestone", id: milestone?.id, projectId: milestone ? String(milestone.projectId) : "", title: milestone?.title ?? "", description: milestone?.description ?? "", severity: "", milestoneDate: milestone?.milestoneDate?.slice(0, 10) ?? "" });
  const saveRecord = () => {
    if (!recordDialog || (recordDialog.type === "milestone" && !recordDialog.title.trim()) || (recordDialog.type === "blocker" && !recordDialog.description.trim())) return;
    const onSuccess = () => { setRecordDialog(null); refresh(recordDialog.type === "blocker" ? blockersKey : milestonesKey); };
    if (recordDialog.type === "blocker") {
      const data = { description: recordDialog.description.trim(), severity: recordDialog.severity };
      if (recordDialog.id) updateBlocker.mutate({ id: recordDialog.id, data }, { onSuccess });
      else if (recordDialog.projectId && data.description) createBlocker.mutate({ projectId: Number(recordDialog.projectId), ...data }, { onSuccess });
    } else {
      const data = { title: recordDialog.title.trim(), description: recordDialog.description.trim() || null, milestoneDate: recordDialog.milestoneDate };
      if (recordDialog.id) updateMilestone.mutate({ id: recordDialog.id, data }, { onSuccess });
      else if (recordDialog.projectId && data.milestoneDate) createMilestone.mutate({ projectId: Number(recordDialog.projectId), ...data }, { onSuccess });
    }
  };
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <Tabs value={tab} onValueChange={setTab}><div className="overflow-x-auto pb-1"><TabsList className="h-auto min-w-max gap-1 p-1"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="people">People & leave</TabsTrigger><TabsTrigger value="projects">Projects & dates</TabsTrigger><TabsTrigger value="calendar">Calendar</TabsTrigger><TabsTrigger value="notices">Notice board</TabsTrigger></TabsList></div>
      <TabsContent value="overview" className="mt-6 space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card>
            <CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-red-600" />Open blockers</CardTitle><CardDescription>Unresolved issues blocking delivery.</CardDescription></div>{canManageRecords && <Button size="sm" variant="outline" onClick={() => openBlockerDialog()}><Plus className="mr-1 h-4 w-4" />Add</Button>}</div></CardHeader>
            <CardContent className="space-y-3">{loadingBlockers ? <p className="py-4 text-center text-sm text-muted-foreground">Loading...</p> : errorBlockers ? <p className="py-4 text-center text-sm text-destructive">Couldn't load blockers.</p> : openBlockers.length === 0 ? <p className="rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">No open blockers.</p> : openBlockers.map((blocker) => { const project = projects.find((item) => item.id === blocker.projectId); return <div className="rounded-lg border p-3" key={blocker.id}><div className="flex items-center justify-between gap-2"><div><p className="font-medium">{blocker.referenceCode}</p><p className="mt-0.5 text-xs text-muted-foreground">{project ? `${project.key} · ${project.name}` : `Project #${blocker.projectId}`}</p></div><div className="flex items-center gap-2"><Badge className={priorityClass(blocker.severity)}>{formatLabel(blocker.severity)}</Badge>{canManageRecords && <><Button size="icon" variant="ghost" className="h-7 w-7" title="Edit blocker" onClick={() => openBlockerDialog(blocker)}><Pencil className="h-3.5 w-3.5" /></Button><Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" title="Delete blocker" onClick={() => window.confirm("Delete this blocker?") && deleteBlocker.mutate(blocker.id, { onSuccess: () => refresh(blockersKey) })}><Trash2 className="h-3.5 w-3.5" /></Button></>}</div></div><p className="mt-1 text-sm text-muted-foreground">{blocker.description}</p></div>; })}</CardContent>
          </Card>
          <Card>
            <CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Flag className="h-4 w-4 text-sky-600" />Upcoming milestones</CardTitle><CardDescription>Dates the Pod is tracking toward.</CardDescription></div>{canManageRecords && <Button size="sm" variant="outline" onClick={() => openMilestoneDialog()}><Plus className="mr-1 h-4 w-4" />Add</Button>}</div></CardHeader>
            <CardContent className="space-y-3">{loadingMilestones ? <p className="py-4 text-center text-sm text-muted-foreground">Loading...</p> : errorMilestones ? <p className="py-4 text-center text-sm text-destructive">Couldn't load milestones.</p> : upcomingMilestones.length === 0 ? <p className="rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">No upcoming milestones.</p> : upcomingMilestones.map((milestone) => <div className="rounded-lg border p-3" key={milestone.id}><div className="flex items-center justify-between gap-2"><p className="font-medium">{milestone.title}</p><div className="flex items-center gap-2"><Badge className={neutralStatusClass(milestone.status)}>{formatLabel(milestone.status ?? "planned")}</Badge>{canManageRecords && <><Button size="icon" variant="ghost" className="h-7 w-7" title="Edit milestone" onClick={() => openMilestoneDialog(milestone)}><Pencil className="h-3.5 w-3.5" /></Button><Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" title="Delete milestone" onClick={() => window.confirm("Delete this milestone?") && deleteMilestone.mutate(milestone.id, { onSuccess: () => refresh(milestonesKey) })}><Trash2 className="h-3.5 w-3.5" /></Button></>}</div></div><p className="mt-1 text-xs text-muted-foreground">{formatDate(milestone.milestoneDate)}</p></div>)}</CardContent>
          </Card>
          <Card>
            <CardHeader><div><CardTitle className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-amber-600" />Conversion reminders</CardTitle><CardDescription>Internship conversion decisions coming due.</CardDescription></div></CardHeader>
            <CardContent className="space-y-3"><div className="rounded-lg border border-dashed p-5 text-center text-sm text-muted-foreground">Coming soon</div></CardContent>
          </Card>
        </div>
      </TabsContent>
    ...
    <Dialog open={!!recordDialog} onOpenChange={(open) => !open && setRecordDialog(null)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{recordDialog?.id ? "Edit" : "Add"} {recordDialog?.type === "blocker" ? "blocker" : "milestone"}</DialogTitle>
        </DialogHeader>
        {recordDialog && <div className="space-y-4">
          {!recordDialog.id && <div className="space-y-2"><label className="text-sm font-medium">Project</label><select value={recordDialog.projectId} onChange={(event) => setRecordDialog({ ...recordDialog, projectId: event.target.value })} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"><option value="">Select a project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.key} - {project.name}</option>)}</select></div>}
          {recordDialog.type === "milestone" && <Input placeholder="Milestone title" value={recordDialog.title} onChange={(event) => setRecordDialog({ ...recordDialog, title: event.target.value })} />}
          {recordDialog.type === "blocker" && <div className="space-y-2"><label className="text-sm font-medium">Severity</label><select value={recordDialog.severity} onChange={(event) => setRecordDialog({ ...recordDialog, severity: event.target.value })} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"><option value="critical">Critical</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></div>}
          {recordDialog.type === "milestone" && <div className="space-y-2"><label className="text-sm font-medium">Date</label><Input type="date" value={recordDialog.milestoneDate} onChange={(event) => setRecordDialog({ ...recordDialog, milestoneDate: event.target.value })} /></div>}
          <Textarea placeholder={recordDialog.type === "blocker" ? "Describe the blocker" : "Description (optional)"} value={recordDialog.description} onChange={(event) => setRecordDialog({ ...recordDialog, description: event.target.value })} />
        </div>}
        <DialogFooter><Button variant="outline" onClick={() => setRecordDialog(null)}>Cancel</Button><Button onClick={saveRecord} disabled={!recordDialog || (recordDialog.type === "blocker" ? !recordDialog.projectId || !recordDialog.description.trim() : !recordDialog.projectId || !recordDialog.title.trim() || !recordDialog.milestoneDate)}>Save</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </div>;
}
